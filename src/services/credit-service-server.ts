...
export async function updateCredit(id: string, creditData: Partial<CreditDetail>, actor: User): Promise<{ success: boolean; error?: string }> {
     try {
        ...
        if (Object.keys(fieldsToUpdate).length > 0) {
            if (fieldsToUpdate.status !== 'Active' && fieldsToUpdate.status !== 'Rejected' && fieldsToUpdate.status !== 'Fallecido' && fieldsToUpdate.status !== 'Approved') {
